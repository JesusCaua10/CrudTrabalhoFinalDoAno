    ...
    <div class="w-full max-w-md bg-white shadow-md rounded-lg p-6">
        ...
        <form action="{{ route('cargo.update', $cargo->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nome" value="{{ old('nome', $cargo->nome) }}" required class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
                @error('nome')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block mb-1 font-medium">Salário</label>
                <input type="number" step="0.01" name="salario" value="{{ old('salario', $cargo->salario) }}" required class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
